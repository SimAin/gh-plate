...
def test_config_file_supplies_owner_alias(
    monkeypatch, tmp_path, capsys, run_with_config
) -> None:
    path = tmp_path / "config.json"
...
def test_owner_flow_never_calls_current_repo(
    ...
    assert (
        run_with_config(prs_cli.run, cli.parse_args(["prs", "--owner", "an-org"])) == 0
    )
    assert "repo-a" in capsys.readouterr().out


def test_owner_groups_output_by_repo(monkeypatch, capsys, run_with_config) -> None:
    prs = [
...
def test_owner_alias_resolves_and_shows_arrow(
    ...
    assert run_with_config(prs_cli.run, cli.parse_args(["prs", "--owner", "work"])) == 0
...
def test_owner_mine_flag_reaches_the_fetch(
    monkeypatch, capsys, run_with_config
) -> None:
    calls = _stub_owner(monkeypatch, prs=[_owner_pr(1)], total=1)
    assert (
        run_with_config(
            prs_cli.run, cli.parse_args(["prs", "--owner", "an-org", "--mine"])
        )
        == 0
    )
...
def test_owner_type_failure_lists_aliases(monkeypatch, capsys, run_with_config) -> None:
    ...
    with pytest.raises(PlateError) as excinfo:
        run_with_config(prs_cli.run, cli.parse_args(["prs", "--owner", "typo"]))
...
def test_owner_type_failure_without_aliases_has_no_alias_line(
    ...
    monkeypatch.setattr(gh, "resolve_owner_type", fail)
    with pytest.raises(PlateError) as excinfo:
        run_with_config(prs_cli.run, cli.parse_args(["prs", "--owner", "nope"]))
    assert "Configured aliases:" not in str(excinfo.value)


def test_owner_missing_login_errors(monkeypatch, run_with_config) -> None:
    from plate.core import config, gh

    monkeypatch.setattr(config, "load_config", lambda *a, **k: config.Config())
    monkeypatch.setattr(gh, "current_login", lambda: None)
    with pytest.raises(PlateError) as excinfo:
        run_with_config(prs_cli.run, cli.parse_args(["prs", "--owner", "an-org"]))
...
def test_owner_truncation_note_limit_hit(monkeypatch, capsys, run_with_config) -> None:
    ...
    assert (
        run_with_config(
            prs_cli.run, cli.parse_args(["prs", "--owner", "an-org", "--limit", "2"])
        )
        == 0
    )
    err = capsys.readouterr().err
...
def test_owner_summary_line_present(monkeypatch, capsys, run_with_config) -> None:
    prs = [
