...
| --- | --- |
| `plate/cli.py` | Top-level wiring only: builds the parser, loads the config once and hands it to a subcommand via a command→runner registry, turns a `PlateError` into a clean exit. Knows nothing about issues, sprints, owners, PRs, or retros — this is the one place the domains may be named. |
...
| `plate/core/text.py` | Data cleaning on the way in from a `gh` payload: `compact_text` (one safe line from untrusted text) and `parse_timestamp`; plus its inverse `format_timestamp` for timestamps going back out (query strings, the JSON envelope). Shared by every domain's `model`. |
...
| `plate/core/flags.py` | The cross-cutting flags, declared once as parent parsers a subcommand mounts: `output()` (`--format`, `--color`) for every view, `config()` (`--config`, `--config-path`) for the views that read the config file. |
| `plate/core/jsonout.py` | The `--format json` envelope every view shares (`envelope`, `dumps`) and `plain`, which turns the model's dataclasses into JSON-ready dicts. Domains supply `data`; the honesty notes ride in the envelope instead of going to stderr. |
